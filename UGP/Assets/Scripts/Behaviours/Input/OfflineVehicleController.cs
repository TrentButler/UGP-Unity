...
namespace UGP
{
    public class OfflineVehicleController : InputController
    {
        ...
        {
            //BOX CAST OUTWARD WITH THE WIDTH OF THE VEHICLE
            //DETERMINE THE ANGLE OF WHAT WE HIT
            //ROTATE THE VEHICLE UPWARDS
            var all_colliders = GetComponentsInChildren<Collider>().ToList();
            var sorted_colliders = all_colliders.OrderBy(x => x.bounds.extents.magnitude).ToList();
            var width = sorted_colliders[0].bounds.extents;

            RaycastHit hit;
            if (Physics.SphereCast(transform.position, width.x, transform.forward.normalized, out hit, AutoRotateThreshold))
            {
                var surface_normal = (hit.normal * (AutoRotateSpeed * Time.smoothDeltaTime));

                var target_rotation = Quaternion.FromToRotation(transform.up, surface_normal);
                rb.MoveRotation(rb.rotation * target_rotation);

                Debug.Log("SURFACE NORM WE HIT: " + surface_normal);
            }          
